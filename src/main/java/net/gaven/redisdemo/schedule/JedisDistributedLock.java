...
public class JedisDistributedLock {
    ...
    DefaultRedisScript<Boolean> luaScript = null;
...
//                redisService.remove(lock);
                 ...
                 * 优化  lua脚本实现锁释放到优化
                 * 由于同key 中有多个值
                 * Server A  key：JOB value：127.0.0.1
                 * Server B  key：JOB value：127.0.0.2
                 * A执行到一半  释放锁  此时B突然就没锁了
                 *
                 */
                System.out.println(getHostIp()+lock);
                luaReleaseLock(lock,getHostIp());
            }
        }
    }

    private boolean luaReleaseLock(String key, String requestIp) {
...
//        luaScript.setLocation(new ClassPathResource("unlock.lua"));
        luaScript.setScriptSource(new ResourceScriptSource(new ClassPathResource("unlock.lua")));

        luaScript.setResultType(Boolean.class);

        Boolean result = redisTemplate.execute(luaScript, list);
        return result;
    }
